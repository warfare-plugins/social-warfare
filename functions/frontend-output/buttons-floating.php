	...
	if ( is_singular() && ! is_attachment() ) :
		...
		if ( is_singular() && get_post_status( $postID ) == 'publish' && get_post_meta( $postID , 'nc_floatLocation' , true ) != 'off' && $visibility == 'on' && ! is_home() ) :
			...
			if ( $options['floating_panel'] && is_singular() ) :
				$float_location =  $options['float_location'];
				$class = "swp_float_" . $options['float_location'];
			else :
				$float_location = 'floatNone';
			endif;
			...
			$assets = '<div class="swp_social_panelSide swp_social_panel swp_
                ' . $options['float_button_shape'] .
                ' swp_d_' . $options['float_default_colors'] .
                ' swp_i_' . $options['float_single_colors'] .
                ' swp_o_' . $options['float_hover_colors'] . '
                ' . $options['transition'] . '
                ' . $class . '
                ' . '" data-position="' . $options['location_post'] .
                '" data-float="' . $float_location .
                '" data-count="' . $buttonsArray['count'] .
                '" data-floatColor="' . $options['float_background_color'] .
                '" data-screen-width="' . $options['float_screen_width'] .
                '" data-transition="' . $options['transition'] .
                '" data-mobileFloat="'.$options['float_mobile'].'">';
