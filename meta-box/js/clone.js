jQuery( function ( $ )
{
	...
	{
		var $clone_last = $container.find( '.SW_META-clone:last' ),
			$clone = $clone_last.clone(),
			$input;

		$clone.insertAfter( $clone_last );
		$input = $clone.find( ':input[class|="SW_META"]' );
		...
		$input.each( function ()
		{
			...
			var $address_button = $clone.find( '.SW_META-map-goto-address-button' );
			if ( $address_button )
			{
	...
	{
		var $clone_first = $container.find( '.SW_META-clone:first' ),
			$clone_last = $container.find( '.SW_META-clone:last' ),
			$clone = $( '<div />' ).addClass( 'SW_META-clone' ),
		...
		if ( typeof SW_META_cloneable_editors !== 'undefined' && typeof SW_META_cloneable_editors[field_id] !== 'undefined' )
		{
			//Get HTML of editor from global object
			var cloned_editor = $( SW_META_cloneable_editors[field_id] );
	...
	$( '.add-clone' ).on( 'click', function ( e )
	{
		...
		var $input = $( this ).parents( '.SW_META-input' );

		if ( $( this ).parents( '.SW_META-field' ).hasClass( 'SW_META-wysiwyg-wrapper' ) )
		{
			cloneWYSIWYG( $input );
		}
		else
	...
	// Remove clones
	$( '.SW_META-input' ).on( 'click', '.remove-clone', function ( e )
	{
		e.preventDefault();

		var $this = $( this ),
			$input = $this.parents( '.SW_META-input' );

		// Remove clone only if there're 2 or more of them
		if ( $input.find( '.SW_META-clone' ).length <= 1 )
		{
			return;
		}
