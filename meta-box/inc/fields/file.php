...
if ( ! class_exists( 'SW_META_File_Field' ) )
{
	class SW_META_File_Field extends SW_META_Field
	{
		/**
		...
		{
			wp_enqueue_style( 'SW_META-file', SW_META_CSS_URL . 'file.css', array(), SW_META_VER );
			wp_enqueue_script( 'SW_META-file', SW_META_JS_URL . 'file.js', array( 'jquery' ), SW_META_VER, true );
			wp_localize_script( 'SW_META-file', 'SW_METAFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload %d image for this field.', 'meta-box' ),
				'maxFileUploadsPlural' => __( 'You may only upload %d images for this field.', 'meta-box' ),
			) );
		}
		...
		static function add_actions()
		{
			...
			add_action( 'wp_ajax_SW_META_delete_file', array( __CLASS__, 'wp_ajax_delete_file' ) );

			// allow reordering
			add_action( 'wp_ajax_SW_META_reorder_files', array( __CLASS__, 'wp_ajax_reorder_files' ) );
		}
		...
		static function wp_ajax_reorder_files()
		{
			...
			check_ajax_referer( "SW_META-reorder-files_{$field_id}" );
		...
		static function wp_ajax_delete_file()
		{
			...
			check_ajax_referer( "SW_META-delete-file_{$field_id}" );
		...
		{
			$i18n_title = apply_filters( 'SW_META_file_upload_string', _x( 'Upload Files', 'file upload', 'meta-box' ), $field );
			$i18n_more  = apply_filters( 'SW_META_file_add_string', _x( '+ Add new file', 'file upload', 'meta-box' ), $field );
		...
		{
			$reorder_nonce = wp_create_nonce( "SW_META-reorder-files_{$field['id']}" );
			$delete_nonce  = wp_create_nonce( "SW_META-delete-file_{$field['id']}" );

			$classes = array( 'SW_META-file', 'SW_META-uploaded' );
			if ( count( $files ) <= 0 )
				$classes[] = 'hidden';
		...
		{
			$i18n_delete = apply_filters( 'SW_META_file_delete_string', _x( 'Delete', 'file upload', 'meta-box' ) );
			$i18n_edit   = apply_filters( 'SW_META_file_edit_string', _x( 'Edit', 'file upload', 'meta-box' ) );
			$li          = '
			<li id="item_%s">
				<div class="SW_META-icon">%s</div>
				<div class="SW_META-info">
					<a href="%s" target="_blank">%s</a>
					<p>%s</p>
					<a title="%s" href="%s" target="_blank">%s</a> |
					<a title="%s" class="SW_META-delete-file" href="#" data-attachment_id="%s">%s</a>
				</div>
			</li>';
