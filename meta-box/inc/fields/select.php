...
if ( ! class_exists( 'SW_META_Select_Field' ) )
{
	class SW_META_Select_Field extends SW_META_Field
	{
		/**
		...
		{
			wp_enqueue_style( 'SW_META-select', SW_META_CSS_URL . 'select.css', array(), SW_META_VER );
		}
		...
		{
			$html = sprintf(
				'<select class="SW_META-select" name="%s" id="%s" size="%s"%s>',
				$field['field_name'],
				$field['id'],
				$field['size'],
