...
if ( ! class_exists( 'SW_META_Datetime_Field' ) )
{
	class SW_META_Datetime_Field extends SW_META_Field
	{
		/**
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_register_script( 'jquery-ui-timepicker', SW_META_JS_URL . 'jqueryui/jquery-ui-timepicker-addon.js', array( 'jquery-ui-datepicker', 'jquery-ui-slider' ), '0.9.7', true );
			...
			{
				if ( file_exists( SW_META_DIR . 'js/' . $date_path ) )
				{
					wp_register_script( 'jquery-ui-datepicker-i18n', SW_META_JS_URL . $date_path, array( 'jquery-ui-datepicker' ), '1.8.17', true );
			...
			{
				if ( file_exists( SW_META_DIR . 'js/' . $time_path ) )
				{
					wp_register_script( 'jquery-ui-timepicker-i18n', SW_META_JS_URL . $time_path, array( 'jquery-ui-timepicker' ), '1.8.17', true );
					$deps[] = 'jquery-ui-timepicker-i18n';
					break;
				}
			}

			wp_enqueue_script( 'SW_META-datetime', SW_META_JS_URL . 'datetime.js', $deps, SW_META_VER, true );
			wp_localize_script( 'SW_META-datetime', 'SW_META_Datetimepicker', array( 'lang' => $locale ) );
		}
		...
		{
			return sprintf(
				'<input type="text" class="SW_META-datetime" name="%s" value="%s" id="%s" size="%s" data-options="%s">',
				$field['field_name'],
