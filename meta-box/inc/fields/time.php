...
if ( ! class_exists( 'SW_META_Time_Field' ) )
{
	class SW_META_Time_Field extends SW_META_Field
	{
		/**
		...
		static function admin_enqueue_scripts()
		{
			...
			if ( file_exists( SW_META_DIR . "js/jqueryui/timepicker-i18n/jquery-ui-timepicker-{$locale}.js" ) )
			{
				$timepicker_locale_js_url = "{$url}/timepicker-i18n/jquery-ui-timepicker-{$locale}.js";
			}
			elseif ( strlen( $locale ) > 2 && file_exists( SW_META_DIR . 'js/jqueryui/timepicker-i18n/jquery-ui-timepicker-' . substr( $locale, 0, 2 ) . '.js' ) )
			{
			...
			wp_enqueue_script( 'SW_META-time', SW_META_JS_URL . 'time.js', $deps, SW_META_VER, true );
			wp_localize_script( 'SW_META-time', 'SW_META_Timepicker', array( 'lang' => $locale ) );
		}
		...
		{
			return sprintf(
				'<input type="text" class="SW_META-time" name="%s" value="%s" id="%s" size="%s" data-options="%s" />',
				$field['field_name'],
				$meta,
