...
if ( ! class_exists( 'SW_META_Helper' ) )
{
	/**
	 * Wrapper class for helper functions
	 */
	class SW_META_Helper
	{
		/**
		...
		{
			add_shortcode( 'SW_META_meta', array( __CLASS__, 'shortcode' ) );
		}
		...
		static function shortcode( $atts )
		{
			...
			return apply_filters( 'SW_META_shortcode', $content );
		}
		...
		static function meta( $key, $args = array(), $post_id = null )
		{
			...
			return apply_filters( 'SW_META_meta', $meta, $key, $args, $post_id );
		}
		...
		static function map( $key, $args = array(), $post_id = null )
		{
			...
			$html = sprintf(
				'<div id="SW_META-map-canvas-%d" style="width:%s;height:%s"></div>',
				$counter,
				$args['width'],
				$args['height']
			);
			...
			$html .= sprintf( '
				...
				map = new google.maps.Map( document.getElementById( "SW_META-map-canvas-%d" ), mapOptions );
				',
				$parts[0], $parts[1],
 ...
 * @return mixed
 */
function SW_META_meta( $key, $args = array(), $post_id = null )
{
	return SW_META_Helper::meta( $key, $args, $post_id );
}
