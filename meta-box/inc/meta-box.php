...
if ( ! class_exists( 'SW_Meta_Box' ) )
{
	...
	class SW_Meta_Box
	{
		...
		function __construct( $meta_box )
		{
			...
			$show = true;
			$show = apply_filters( 'SW_META_show', $show, $this->meta_box );
			$show = apply_filters( "SW_META_show_{$this->meta_box['id']}", $show, $this->meta_box );
			if ( ! $show )
				return;
		...
		function admin_enqueue_scripts()
		{
			...
			wp_enqueue_style( 'SW_META', SW_META_CSS_URL . 'style.css', array(), SW_META_VER );
			...
			if ( $has_clone )
				wp_enqueue_script( 'SW_META-clone', SW_META_JS_URL . 'clone.js', array( 'jquery' ), SW_META_VER, true );

			if ( $this->validation )
			{
				wp_enqueue_script( 'jquery-validate', SW_META_JS_URL . 'jquery.validate.min.js', array( 'jquery' ), SW_META_VER, true );
				wp_enqueue_script( 'SW_META-validate', SW_META_JS_URL . 'validate.js', array( 'jquery-validate' ), SW_META_VER, true );
			}

			// Auto save
			if ( $this->meta_box['autosave'] )
				wp_enqueue_script( 'SW_META-autosave', SW_META_JS_URL . 'autosave.js', array( 'jquery' ), SW_META_VER, true );
		}
		...
		function show()
		{
			...
			// Container
			printf(
				'<div class="SW_META-meta-box" data-autosave="%s">',
				$this->meta_box['autosave'] ? 'true' : 'false'
			);

			wp_nonce_field( "SW_META-save-{$this->meta_box['id']}", "nonce_{$this->meta_box['id']}" );

			// Allow users to add custom code before meta box content
			// 1st action applies to all meta boxes
			// 2nd action applies to only current meta box
			do_action( 'SW_META_before', $this );
			do_action( "SW_META_before_{$this->meta_box['id']}", $this );
			...
			if ( isset( $this->validation ) && $this->validation )
			{
				echo '
					...
					else
					{
						...
						jQuery.extend( true, SW_META.validationOptions, tempOptions );
					}
					</script>
				';
			}
			...
			// 1st action applies to all meta boxes
			// 2nd action applies to only current meta box
			do_action( 'SW_META_after', $this );
			do_action( "SW_META_after_{$this->meta_box['id']}", $this );
		...
		function save_post( $post_id )
		{
			...
			if ( empty( $_POST["nonce_{$id}"] ) || ! wp_verify_nonce( $nonce, "SW_META-save-{$id}" ) )
				return;
			...
			// Before save action
			do_action( 'SW_META_before_save_post', $post_id );
			do_action( "SW_META_{$this->meta_box['id']}_before_save_post", $post_id );
			...
			foreach ( $this->fields as $field )
			{
				...
				$new = apply_filters( "SW_META_{$field['type']}_value", $new, $field, $old );
				$new = apply_filters( "SW_META_{$name}_value", $new, $field, $old );
			...
			// After save action
			do_action( 'SW_META_after_save_post', $post_id );
			do_action( "SW_META_{$this->meta_box['id']}_after_save_post", $post_id );
		}
		...
		static function normalize( $meta_box )
		{
			...
			$meta_box = apply_filters( 'SW_META_normalize_meta_box', $meta_box );
			$meta_box = apply_filters( "SW_META_normalize_{$meta_box['id']}_meta_box", $meta_box );
		...
		static function normalize_fields( $fields )
		{
			foreach ( $fields as &$field )
			{
				...
				$field = apply_filters( 'SW_META_normalize_field', $field );
				$field = apply_filters( "SW_META_normalize_{$field['type']}_field", $field );
				$field = apply_filters( "SW_META_normalize_{$field['id']}_field", $field );
			}
		...
		static function get_class_name( $field )
		{
			...
			$class = 'SW_META_' . ucwords( $type ) . '_Field';
